...
from save_checkpoints import CheckpointPercent
...
class LitCycleGAN(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        # Initialize Networks
        self.gen_AB = torch.compile(define_generator(self.hparams, "A"))
        self.gen_BA = torch.compile(define_generator(self.hparams, "B"))
        
        
        if self.hparams.enable_val_metrics:
            import torchmetrics.image as tmi
            self.fid = tmi.fid.FrechetInceptionDistance(normalize=True, reset_real_features=False)
            self.kid = tmi.kid.KernelInceptionDistance(normalize=True,
                                                       subset_size=int(self.hparams.batch_size_val/4),
                                                       subsets=self.hparams.batch_size_val,
                                                       reset_real_features=False)
            self.lpips = tmi.lpip.LearnedPerceptualImagePatchSimilarity(normalize=True)
            self.insc = tmi.inception.InceptionScore(normalize=True)
        ...
        # create image pools
        self.pool_fakeA = ImagePool(100)
        self.pool_fakeB = ImagePool(100)
    ...
    def on_train_epoch_start(self):
        ...
        if self.hparams["inpainting_start_epoch"] > self.current_epoch:
            self.lambda_inpainting = 0.0  # not needed since inpainting images are not created, but to be consistent
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 1:
            ...
            # query fake images from pool
            fake_A = self.pool_fakeA.query(self.fake_A).detach()
            fake_B = self.pool_fakeB.query(self.fake_B).detach()
    ...
    def validation_step(self, batch, batch_idx):
        if self.hparams.enable_val_metrics:
            # Generate validation metrics which need synthetic images
            # none at the moment
            synth_B = self.gen_AB(batch["A"])
            real_B = ((batch["B"]+1)/2).expand(-1,3,-1,-1)
            synth_B = ((synth_B+1)/2).expand(-1,3,-1,-1)
            
            if self.trainer.current_epoch == 0:
                # only calculate fid and kid for real images in first epoch since it is saved (reset_real_features=False)
                self.fid.update(real_B, real=True)
                self.kid.update(real_B, real=True)
            
            t_start = perf_counter()
            self.fid.update(synth_B, real=False)
            self.val_metrics["fid_time_update"].append(perf_counter()-t_start)
            
            t_start = perf_counter()
            self.kid.update(synth_B, real=False)
            self.val_metrics["kid_time_update"].append(perf_counter()-t_start)
            
            t_start = perf_counter()   
            self.lpips.update(real_B, synth_B)
            self.val_metrics["lpips_time_update"].append(perf_counter()-t_start)
            
            t_start = perf_counter()
            self.insc.update(synth_B)
            self.val_metrics["insc_time_update"].append(perf_counter()-t_start)
        return
    
    def on_validation_epoch_end(self):
        if self.hparams.enable_val_metrics:
            # Generate validation metrics which do not need synthetic images and merge metrics from steps
            t1_start = perf_counter()
            watcher = ww.WeightWatcher(model=self.gen_AB)
            desc = watcher.describe()
            res = watcher.analyze()
            summary = watcher.get_summary()
            for key, value in summary.items():
                self.val_metrics["gen_AB_"+key].append(value)
            
            watcher = ww.WeightWatcher(model=self.gen_BA)
            watcher.analyze()
            summary = watcher.get_summary()
            for key, value in summary.items():
                self.val_metrics["gen_BA_"+key].append(value)
                
            for idx, disc_A in enumerate(self.disc_A):
                watcher = ww.WeightWatcher(model=disc_A)
                watcher.analyze()
                summary = watcher.get_summary()
                for key, value in summary.items():
                    self.val_metrics["disc_A_"+str(idx)+"_"+key].append(value)
            for idx, disc_B in enumerate(self.disc_B):
                watcher = ww.WeightWatcher(model=disc_B)
                watcher.analyze()
                summary = watcher.get_summary()
                for key, value in summary.items():
                    self.val_metrics["disc_B_"+str(idx)+"_"+key].append(value)     
            self.val_metrics["ww_time"].append(perf_counter()-t1_start)
            
            t_start = perf_counter()
            self.val_metrics["fid"].append(self.fid.compute())
            self.val_metrics["fid_time_compute"].append(perf_counter()-t_start)
            
            t_start = perf_counter()
            kid_metric = self.kid.compute()
            self.val_metrics["kid_mean"].append(kid_metric[0])
            self.val_metrics["kid_std"].append(kid_metric[1])
            self.val_metrics["kid_time_compute"].append(perf_counter()-t_start)
            
            t_start = perf_counter()
            self.val_metrics["lpips"].append(self.lpips.compute())
            self.val_metrics["lpips_time_compute"].append(perf_counter()-t_start)  
            
            t_start = perf_counter()
            insc_metric = self.insc.compute()
            self.val_metrics["insc_mean"].append(insc_metric[0])
            self.val_metrics["insc_std"].append(insc_metric[1])
            self.val_metrics["insc_time_compute"].append(perf_counter()-t_start)
            
            # explicitly log metrics for callbacks with pytorch to make them available to callbacks
            self.log("gen_AB_alpha", self.val_metrics["gen_AB_alpha"][-1])
            self.log("kid_mean", self.val_metrics["kid_mean"][-1])
            self.log("fid", self.val_metrics["fid"][-1])     
            
            self.log_new_metrics()
        
    def log_new_metrics(self):
        new_metrics = defaultdict()
        for key, value in self.val_metrics.items():
            new_metrics["val/"+key] = value[-1]
        wandb.log(new_metrics)  # use wandb to log dict
        

    @staticmethod
    def add_model_specific_args(parent_parser):
        parser = ArgumentParser(parents=[parent_parser], add_help=False)
        parser.add_argument('--num_channels_A', type=int, default=1, help="Number of channels of images in domain A. --lambda_idt has to be set to zero, if num channels in A and B do not match.")
        parser.add_argument('--num_channels_B', type=int, default=1, help="Number of channels of images in domain B. --lambda_idt has to be set to zero, if num channels in A and B do not match.")
        ...
        parser.add_argument('--lambda_inpainting', type=float, default=10.0, help="Scaling factor for inpainting loss criterion.")
        ...
        parser.add_argument('--enable_val_metrics', '-evm', default=False, action='store_true', help="Enable logging of validation metrics")
        return parser
...
class WandbImageCallback(pl.Callback):
    ...
    def __init__(self, num_val_samples=2, val_img_frequence=1):
        super().__init__()
        self.num_val_samples = num_val_samples
        self.val_img_frequence = val_img_frequence
        self.test_step = 0

    def on_train_epoch_end(self, trainer, pl_module):
        if self.num_val_samples == 0:
            return
        if (trainer.current_epoch % self.val_img_frequence) != 0:
            return
        outputs = []
    ...
    def scale_outputs(self, outputs, input_norm_A, input_norm_B):
        ...
        for output in outputs:
            ...
            if norm_inpainting in output.keys():
                input_norm_inpainting = (np.min([input_norm_A[0], input_norm_B[0]]), np.max([input_norm_A[1], input_norm_B[1]]))
                output[norm_inpainting] = (output[norm_inpainting]-input_norm_inpainting[0])*255/(input_norm_inpainting[1]-input_norm_inpainting[0])  # (X-Xmin)*255/(Xmax-Xmin)
        return outputs
...
def train(args):
    ...
    data = get_data_module(args)

    checkpoint_callbacks = [pl.callbacks.ModelCheckpoint(dirpath="logs/"+args.project+"/"+run.name,
                                                         every_n_epochs=args.checkpoint_every_n_epochs, save_top_k=-1,
                                                         save_weights_only=True)]
    
    if args.enable_val_metrics:
        checkpoint_callbacks.append(pl.callbacks.ModelCheckpoint(dirpath="logs/"+args.project+"/"+run.name,
                                                                 filename='{epoch}-{gen_AB_alpha:.2f}',
                                                                 monitor="gen_AB_alpha", save_top_k=1,
                                                                 save_weights_only=True))
        checkpoint_callbacks.append(CheckpointPercent(dirpath="logs/"+args.project+"/"+run.name,
                                                      filename='percent_{epoch}-{gen_AB_alpha:.2f}',
                                                      monitor="gen_AB_alpha", percent = args.checkpoint_percent_decrease, 
                                                      save_top_k=-1, save_weights_only=True))
        
        checkpoint_callbacks.append(pl.callbacks.ModelCheckpoint(dirpath="logs/"+args.project+"/"+run.name,
                                                                 filename='{epoch}-{kid_mean:.2f}',
                                                                 monitor="kid_mean", save_top_k=1,
                                                                 save_weights_only=True))
        checkpoint_callbacks.append(CheckpointPercent(dirpath="logs/"+args.project+"/"+run.name,
                                                      filename='percent_{epoch}-{kid_mean:.2f}',
                                                      monitor="kid_mean", percent = args.checkpoint_percent_decrease, 
                                                      save_top_k=-1, save_weights_only=True))
        
        checkpoint_callbacks.append(pl.callbacks.ModelCheckpoint(dirpath="logs/"+args.project+"/"+run.name,
                                                                 filename='{epoch}-{fid:.2f}',
                                                                 monitor="fid", save_top_k=1,
                                                                 save_weights_only=True))
        checkpoint_callbacks.append(CheckpointPercent(dirpath="logs/"+args.project+"/"+run.name,
                                                      filename='percent_{epoch}-{fid:.2f}',
                                                      monitor="fid", percent = args.checkpoint_percent_decrease, 
                                                      save_top_k=-1, save_weights_only=True))
    ...
    trainer = pl.Trainer.from_argparse_args(args, logger=wandb_logger, accelerator='gpu', devices=[0],
                                            max_epochs=args.n_epochs_lr_fix+args.n_epochs_lr_decay,
                                            num_sanity_val_steps=0,
                                            callbacks=[WandbImageCallback(args.num_val_samples, args.val_img_frequence),
...
def start_from_ckpt(args):
    ...
    if args.enable_val_metrics:
        checkpoint_callbacks.append(pl.callbacks.ModelCheckpoint(dirpath="logs/"+args.project+"/"+run.name,
                                                                 filename='{epoch}-{gen_AB_alpha:.2f}',
                                                                 monitor="gen_AB_alpha", save_top_k=5,
                                                                 save_weights_only=True))
    ...
    trainer = pl.Trainer.from_argparse_args(args, logger=wandb_logger, accelerator='gpu', devices=[0],
                                            ...
                                            callbacks=[WandbImageCallback(args.num_val_samples, args.val_img_frequence),
                                                       pl.callbacks.LearningRateMonitor(logging_interval='step'),
                                                       pl.callbacks.RichProgressBar(refresh_rate_per_second=1),
                                                       checkpoint_callbacks])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', default=4, type=int, help="Batch size fed into the network")
    parser.add_argument('--batch_size_val', default=40, type=int, help="Validation batch size fed into the network. Only valid when --enable_val_metrics is set.")
    ...
    parser.add_argument('--path_train_A', default="", type=str, help="Path to training images from domain A, when using the bbbc039 data module. Path has to be absolute.")
    parser.add_argument('--path_train_B', default="", type=str, help="Path to training images from domain B, when using the bbbc039 data module. Path has to be absolute")
    parser.add_argument('--path_val_A', default="", type=str, help="Path to val images from domain A, when using the bbbc039 data module. Path has to be absolute. Only valid when --enable_val_metrics is set.")
    parser.add_argument('--path_val_B', default="", type=str, help="Path to val images from domain B, when using the bbbc039 data module. Path has to be absolute. Only valid when --enable_val_metrics is set.")
    parser.add_argument('--path_test_A', default="", type=str, help="Path to test images from domain A, when using the bbbc039 data module. Path has to be absolute.")
    parser.add_argument('--path_test_B', default="", type=str, help="Path to test images from domain B, when using the bbbc039 data module. Path has to be absolute.")
    ...
    parser.add_argument('--val_img_frequence', type=int, default=1, help='Frequency in which validation image are generated.')
    parser.add_argument('--checkpoint_percent_decrease','-cpd', type=float, default=5, help="Percentage of minimal decrease in metric needed before a new checkpoint is saved.")
    parser.add_argument('--checkpoint_every_n_epochs', '-cpn', type=int, default=25, help="Checkpoint every n epochs.")
